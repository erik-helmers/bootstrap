...
let doc_of_name i name =
  match name with
  | Global id -> !^id
  | Quote j | Local j -> char @@ char_of_int (j - i + 97)

let rec protect i expr =
  if is_atom expr then doc_of_expr i expr else parens (doc_of_expr i expr)
...
and doc_of_expr =
  let rec aux i (expr : expr) =
    ...
    | EFun f ->
        !^"fun" ^/^ doc_of_name i (Local 0) ^/^ !^"->" ^//^ aux (i + 1) f
    | EPi (a, b) ->
        pi ^/^ doc_of_name i (Local 0) ^/^ !^":"
        ^/^ protect (i + 1) a
        ^/^ !^"."
        ^//^ aux (i + 1) b
    | ESig (a, b) ->
        si ^/^ doc_of_name i (Local 0) ^/^ !^":"
        ^/^ protect (i + 1) a
        ^/^ !^"."
        ^//^ aux (i + 1) b
  in
...
let print_expr expr =
  ToChannel.pretty 0.80 80 Out_channel.stdout (doc_of_expr 0 expr)
